...
- **Released versions** on CRAN use three components: `X.Y.Z` (e.g. `0.5.0`).
...
So CRAN has `0.5.0`, and the `main` branch reads `Version: 0.5.0.9000` for the entire time we are working toward the next release. The `.9000` suffix is a signal that says "this is a dev build, not a CRAN release." When the maintainer is ready to submit the next release to CRAN, the version is bumped to `0.6.0` (dropping `.9000`), submitted, and then immediately bumped back to `0.6.0.9000` on `main` for the next dev cycle.
