...
they are doing — every method documents the function it calls, and `$fit`
hands you the object that function returned. What tidylearn adds is one
signature across all 20 methods, and output that is already a tibble or a
ggplot2 object, so results move into dplyr and the rest of the tidyverse
without conversion.
